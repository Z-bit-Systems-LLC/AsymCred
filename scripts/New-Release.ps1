...
<#
.SYNOPSIS
    Cut an AsymCred release: bump the version, commit, tag, and push.
.DESCRIPTION
    Modelled on OSDP-Embedded's script of the same name. It automates the
    *local* half of the release — it publishes nothing. Pushing the
    `v<version>` tag triggers the Azure pipeline
    (ci/azure-pipelines.yml); the human-facing GitHub Release is a
    separate, later step (scripts/Publish-GitHubRelease.ps1).

    AsymCred ships no package to any registry — no crates.io, no npm, no
    binaries — so unlike OSDP-Embedded there is no irreversible publish
    and no approval-gated Release pipeline in the middle. The release is
    the tag plus the GitHub Release notes; GitHub generates the source
    archives from the tag. See docs/RELEASING.md.

    Steps, in order:
      1. Validate: inside a git repo, on `main`, with a clean working
         tree.
      2. Sync: fetch origin and fast-forward `main` so the tag sits on
         top of what's published.
      ...
      5. Bump (Set-Version.ps1 — updates CMakeLists.txt).
    ...
    -DryRun walks every step and prints what it would do without
    writing, committing, tagging, or pushing anything.
...
.PARAMETER Version
    Explicit version to release (e.g. '0.2.0'). Takes precedence over
    -IncrementType. Must be numeric SemVer — pre-release suffixes are
    not supported in this repo (see Set-Version.ps1).
.PARAMETER DryRun
...
.PARAMETER SkipChecks
    ...
    before the push — there is no git pre-push hook — so the tagged
    commit's correctness then rests entirely on what you verified
    beforehand and on the pipeline run the tag triggers.
.EXAMPLE
...
.EXAMPLE
    ...
    Cut 0.1.0 -> 0.2.0.
.EXAMPLE
    ./scripts/New-Release.ps1 -Version 1.0.0 -AutoConfirm
#>
...
try {
    ...
    if ($Version) {
        ...
        if ($current -notmatch '^(\d+)\.(\d+)\.(\d+)$') {
            throw "Cannot parse current version '$current' as numeric SemVer."
        }
    ...
    if ($newVersion -notmatch '^\d+\.\d+\.\d+$') {
        throw ("Resolved version '$newVersion' is not numeric SemVer. Pre-release " +
               "suffixes are not supported in this repo — see scripts/Set-Version.ps1.")
    }
    $tag = "v$newVersion"
    ...
    if ($LASTEXITCODE -eq 0 -and $lastTag) {
        & git --no-pager log --oneline "$(($lastTag | Out-String).Trim())..HEAD"
    }
    else {
    ...
    Write-Warn 'That triggers the Azure pipeline. Nothing is published to any registry;'
    Write-Warn 'the release becomes visible when you run Publish-GitHubRelease.ps1.'
    ...
    elseif ($DryRun) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw ("Verification failed. The version bump is staged in your working " +
                   ...
                   "-SkipChecks) and try again. 'git checkout -- CMakeLists.txt' " +
                   "reverts the bump.")
        }
    }
    ...
    if ($DryRun) {
        Write-Info '  [dry-run] git add CMakeLists.txt'
    ...
    Invoke-Git add CMakeLists.txt | Out-Null
    ...
    # The release is NOT finished here. This script only cuts the tag;
    # two steps remain and neither is automated, so spell both out. In
    # OSDP-Embedded, printing only part of the remaining process is how
    # v1.0.0 shipped without a GitHub Release.
    Write-Host ''
    Write-Ok "Release $newVersion cut. Two steps remain:"
    Write-Host ''
    Write-Info '  2/3  Wait for the Azure pipeline on the tag to go green.'
    Write-Info '       If it fails, fix forward and cut a new patch version —'
    Write-Info '       do not move the tag.'
    Write-Host ''
    Write-Info '  3/3  Publish the GitHub Release:'
    Write-Host ''
    ...
    Write-Info '       supply written prose instead (see docs/RELEASING.md).'
    Write-Host ''
