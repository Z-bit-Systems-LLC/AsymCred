...
<#
.SYNOPSIS
    Publish a GitHub Release for an already-pushed AsymCred tag.
.DESCRIPTION
    Step 3 of the release process (see docs/RELEASING.md):

      1. ./scripts/New-Release.ps1              — bump, commit, tag, push.
      2. Wait for the Azure pipeline on the tag to go green.
      3. ./scripts/Publish-GitHubRelease.ps1    — this script.

    New-Release.ps1 deliberately stops at the push: the tag is what
    triggers the pipeline, and the human-facing GitHub Release is a
    separate, later decision that should only be made once the tag built
    clean. This script closes that gap. It never creates or moves a tag —
    it only publishes a release for one that is already on origin.

    Steps, in order:
      1. Validate: inside a git repo; `gh` is installed and
         authenticated; the tag exists locally AND on origin; no GitHub
         Release exists for it yet (this script never clobbers one).
      2. Resolve the previous tag (`git describe --tags --abbrev=0
         <tag>^`) to bound the commit range. No previous tag means the
         whole history — which is the case for AsymCred's first release.
    ...
    No binaries are attached. AsymCred is a source library with no
    packaged artifact (see ci/azure-pipelines.yml on why there is no
    package job); GitHub attaches the source archives for the tag by
    itself.
.PARAMETER Tag
    The tag to publish, e.g. 'v0.2.0'. Defaults to the most recent tag
...
.PARAMETER NotesFile
    ...
    Keep such files under docs/release-notes/.
.PARAMETER Draft
...
.EXAMPLE
    ...
    Publish v0.2.0 after confirming at the prompt.
.EXAMPLE
...
$ghRepo  = 'Z-bit-Systems-LLC/AsymCred'
...
# Render order. 'Other' is last on purpose — it is the catch-all, and
# AsymCred's history so far is written in plain imperative subjects
# rather than Conventional Commits, so expect it to carry most of them.
...
function New-ReleaseNotes {
    ...
    if ($PrevTag) {
        ...
        # No earlier tag to compare against — link the commit list
        # instead so the line is never a broken compare URL.
...
try {
    ...
    $prevTag = $null
    # `<tag>^` fails if the tagged commit is a root commit — then there
    # is no earlier history to describe and the whole log is the range.
    ...
    if ($prevTag) {
        ...
        # Expected for AsymCred's first release.
        $range = $Tag
    ...
    # Computed even for the dry run so the printed command is the real
    # one.
    $tempPath = Join-Path ([System.IO.Path]::GetTempPath()) "asymcred-release-$Tag-$([guid]::NewGuid().ToString('N')).md"
